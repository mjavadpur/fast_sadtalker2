...
class Audio2Coeff():
    def __init__(self, sadtalker_path, device):
        ...
        #     self.audio2pose_model.netG.decoder = nn.DataParallel(self.audio2pose_model.netG.decoder)
        #     # self.audio2pose_model.netD_motion = nn.DataParallel(self.audio2pose_model.netD_motion)
        ...
        #     # checkpoint = torch.load('./sadtalker_expnet.pth')
        #     checkpoint = torch.load('./checkpoint.pth')
        # else:
        #     # checkpoint = torch.load('./sadtalker_expnet.pth', map_location=lambda storage, loc: storage)
        ...
        for param in self.audio2exp_model.parameters():
            param.requires_grad = False
        # if torch.cuda.device_count() > 1:
        #     self.audio2exp_model.netG.audio_encoder = nn.DataParallel(self.audio2exp_model.netG.audio_encoder)
        #     self.audio2exp_model.netG.mapping1 = nn.DataParallel(self.audio2exp_model.netG.mapping1)
    ...
    def using_refpose(self, coeffs_pred_numpy, ref_pose_coeff_path):
        ...
        refpose_coeff = refpose_coeff_dict['coeff_3dmm'][:,64:70] # the indices of pose coeff
