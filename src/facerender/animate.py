...
class AnimateFromCoeff():
    ...
    def __init__(self, sadtalker_path, device):
        ...
        if torch.cuda.device_count() > 1:
            kp_extractor = nn.DataParallel(kp_extractor)
            generator = nn.DataParallel(generator)
            # he_estimator = nn.DataParallel(he_estimator)
    ...
    def generate(self, x, video_save_dir, pic_path, crop_info, fps, enhancer=None, background_enhancer=None, preprocess='crop', img_size=256):
        ...
        # add
        # ratio = predictions_video.shape[1] / target_semantics.shape[1]
        # draw_frame_num = int(frame_num * ratio)

        predictions_video = predictions_video.reshape((-1,)+predictions_video.shape[2:])
        predictions_video = predictions_video[:frame_num]
        # predictions_video = predictions_video[:draw_frame_num]
        ...
        # imageio.mimsave(path, result,  fps=float(25 * ratio))
        ...
        # frames = draw_frame_num
        # end_time = start_time + frames*1/(25*ratio)*1000
